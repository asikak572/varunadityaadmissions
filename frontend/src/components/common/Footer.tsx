  ...
  return (
    <footer className="bg-navy text-white py-12 mt-auto">
      <div className="max-w-7xl mx-auto px-4 grid grid-cols-1 md:grid-cols-4 gap-8">
        ...
        <div>
          ...
          <ul className="space-y-2 text-sm text-gray-400">
            {[['/', 'Home'], ['/colleges', 'Top Colleges'], ['/courses', 'Courses'], ['/apply', 'Apply Now']].map(([href, label]) => (
              <li key={href}>
        ...
        <div>
          ...
          <ul className="space-y-2 text-sm text-gray-400">
            {['Engineering', 'Medical', 'Management', 'Law', 'Design'].map((s) => (
              <li key={s}>
                <Link href={`/courses?stream=${s.toLowerCase()}`} className="hover:text-gold transition-colors">{s}</Link>
              </li>
            ))}
          </ul>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 mt-8 pt-6 border-t border-white/10 text-center text-gray-500 text-sm">
        &copy; {new Date().getFullYear()} Varun Aditya Admissions & Career Guidance. All rights reserved.
      </div>
    </footer>
  )
}
